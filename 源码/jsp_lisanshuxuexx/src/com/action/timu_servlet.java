...
public class timu_servlet extends HttpServlet
{
	...
	public void timuSuiji(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
	{
		...
		int[] timu_id=liuService.Random(timu_idList, 5);//随机抽取不同的3个题目的id
		...
		mydb.closed();
		req.getSession().setAttribute("jiandatiList", jiandatiList);
	...
	public void timuSuiji_defen(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
	{
		...
		List jiandatiList=(List)req.getSession().getAttribute("jiandatiList");		
		for(int i=0;i<jiandatiList.size();i++)
		{
			Tjiandati jiandati=(Tjiandati)jiandatiList.get(i);			
			String timuDaan=jiandati.getDaan();
			String timuDaan_user=req.getParameter(String.valueOf(jiandati.getId()));
			if(timuDaan.equalsIgnoreCase(timuDaan_user))
			{
				fenshu+=35;
			}
